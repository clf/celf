;;; celf-font.el.  Highlighting for Celf mode using font-lock in XEmacs
;;;
...
(defun celf-font-create-face (face from-face color)
  "Creates a Celf font from FROM-FACE with COLOR."
  (make-face face)
  ;(reset-face face)			; seems to be necessary, but why?
  (copy-face from-face face)
  (if color (set-face-foreground face color)))

(defvar celf-font-dark-background nil
...
(cond (celf-font-dark-background
       (celf-font-create-face 'celf-font-keyword-face 'font-lock-comment-face nil)
       (celf-font-create-face 'celf-font-const-face 'font-lock-comment-face nil)
       (celf-font-create-face 'celf-font-comment-face  'font-lock-comment-face
			     nil)
       (celf-font-create-face 'celf-font-percent-key-face 'font-lock-comment-face "Plum")
       (celf-font-create-face 'celf-font-decl-face 'font-lock-comment-face "Orange")
       (celf-font-create-face 'celf-font-parm-face 'font-lock-comment-face "Orange")
       (celf-font-create-face 'celf-font-fvar-face 'font-lock-comment-face "SpringGreen")
       (celf-font-create-face 'celf-font-evar-face 'font-lock-comment-face "Aquamarine"))
      (t 
       (celf-font-create-face 'celf-font-keyword-face 'font-lock-comment-face nil)
       (celf-font-create-face 'celf-font-const-face 'font-lock-comment-face nil)
       (celf-font-create-face 'celf-font-comment-face 'font-lock-comment-face
			     nil)
       (celf-font-create-face 'celf-font-percent-key-face 'font-lock-comment-face "MediumPurple")
       (celf-font-create-face 'celf-font-decl-face 'font-lock-comment-face "FireBrick")
       (celf-font-create-face 'celf-font-parm-face 'font-lock-comment-face "Green4")
       (celf-font-create-face 'celf-font-fvar-face 'font-lock-comment-face "Blue1")
       (celf-font-create-face 'celf-font-evar-face 'font-lock-comment-face "Blue4")))

;; Note that the order matters!

(defvar celf-font-patterns
 '(
   ;; delimited comments, perhaps should use different font
   ;;("%{" "}%" comment)
   (celf-font-find-delimited-comment . celf-font-comment-face)
   ;; empty-line comments
   ("%$" 0 celf-font-comment-face)
   ;; single-line comments
   ("%[% \t\f].*$" 0 celf-font-comment-face)
   ...
   ("\\(#infix\\|#prefix\\|#postfix\\|#name\\|#freeze\\|#thaw\\|#abbrev\\|#clause\\|#define\\|#solve\\|#querytabled\\|#query\\|#tabled\\|#deterministic\\|#mode\\|#unique\\|#block\\|#worlds\\|#covers\\|#total\\|#terminates\\|#reduces\\|#theorem\\|#prove\\|#assert\\|#establish\\|#sig\\|#struct\\|#trustme\\|#where\\|#include\\|#open\\|#use\\).*$"
    1 celf-font-percent-key-face nil)
...
;;"\\(\\<<-\\>\\|\\<->\\>\\|\\<o-\\>\\|\\<-o\\>\\|\\<<T>\\>\\|\\<&\\>\\|\\^\\|()\\|\\<type\\>\\|\\<sigma\\>\\)"
    ...
    ;;"\\([][:.(){}]\\|\\<<-\\>\\|\\<->\\>\\|\\<type\\>\\|\\<sigma\\>\\)"
   . celf-font-keyword-face)
   ;; declared constants
   (celf-font-find-decl . celf-font-decl-face)
   ;; parameters
   (celf-font-find-parm . celf-font-parm-face)
   ;; quantified existentials
   (celf-font-find-evar . celf-font-evar-face)
   ;; lower-case identifiers (almost = constants)
   ;;("\\<\\([a-z!&$^+/<=>?@~|#*`;,]\\|\\-\\|\\\\\\)\\w*\\>"
   ;; nil black)
   ;; qualified identifiers
   ("\\<\\w+\\(\\.\\w+\\)+\\>" . celf-font-const-face)
   ;; upper-case identifiers (almost = variables)
   ("\\<[A-Z_]\\w*\\>" . celf-font-fvar-face)
   ;; numbers and quoted identifiers omitted for now
   )
 "Highlighting patterns for Celf mode.
...
;Used to uninstall immediate highlighting in Celf mode.")

;(defun toggle-celf-font-immediate ()
;  "Toggle immediate highlighting option in Celf mode.
;This is tied to font-lock mode."
;  (interactive)
;  (if font-lock-mode
;      (celf-font-uninstall-immediate)
;    (celf-font-install-immediate)))

;(defun celf-font-install-immediate ()
...
;  (fset 'font-lock-fontify-region 'celf-font-hacked-fontify-region)
;  (font-lock-mode t))

;(defun celf-font-uninstall-immediate ()
...
;(defun celf-font-hacked-fontify-region (start end)
;  (if (eq major-mode 'celf-mode)
;      (celf-font-fontify-region start end)
;    (funcall font-lock-fontify-region-original start end)))
;;; End experimental section

(defun celf-font-fontify-decl  ()
  "Fontifies the current Celf declaration."
  (interactive)
  (let* ((inhibit-read-only t)		; for FSF Emacs
	 (region (celf-current-decl))
	 (start (nth 0 region))
	 (end (nth 1 region)))
    (save-excursion
...
(defun celf-font-fontify-buffer ()
  "Fontitifies the current buffer as Celf code."
  (interactive)
  (let ((inhibit-read-only t))		; for FSF Emacs
    (if (not celf-config-mode)
	(save-excursion
	  (font-lock-unfontify-region (point-min) (point-max)) ; t optional in XEmacs
	  (celf-font-fontify-region (point-min) (point-max))))))

(defun celf-font-unfontify ()
...
(defun celf-font-fontify-region (start end)
  "Go through CELF-FONT-PATTERNS, fontifying according to given functions"
  (save-restriction
    (narrow-to-region start end)
    (if (and font-lock-verbose
	     (>= (- end start) font-lock-message-threshold))
	(message "Fontifying %s... (semantically...)" (buffer-name)))
    (let ((patterns celf-font-patterns)
	  (case-fold-search nil)	; in Celf, never case-fold
	...
	(cond ((symbolp fun-or-regexp)	; a function to call
		 ...
		 (celf-font-highlight (car region) (cdr region)
		 ...
		 (celf-font-highlight (match-beginning match-index)
...
(defun celf-font-highlight (start end face allow-overlap-p)
...
(defun celf-font-find-delimited-comment (limit)
  "Find a delimited Celf comment and return (START . END), nil if none."
...
(defun celf-font-find-decl (limit)
  "Find an Celf constant declaration and return (START . END), nil if none."
  (let (start
	end
	;; Turn off error messages
	(id (celf-font-next-decl nil nil)))
	...
	nil
      ...
      (setq start (point))
      (celf-font-end-of-par)
      (cons start end))))

(defun celf-font-find-binder (var-pattern limit occ-face)
  "Find Celf binder whose bound variable matches var-pattern.
...
(defun celf-font-find-parm (limit)
  "Find bound Celf parameters and return (START . END), NIL if none.
Also highlights all occurrences of the parameter.
For these purposes, a parameter is a bound, lower-case identifier."
  (celf-font-find-binder "\\<[-a-z!&$^+/\\<=>?@~|#*`;,]\\w*\\>"
			limit 'celf-font-parm-face))

(defun celf-font-find-evar (limit)
  "Find bound Celf existential variable return (START . END), NIL if none.
...
For these purposes, an existential variable is a bound, upper-case identifier."
  (celf-font-find-binder "\\<[A-Z_]\\w*\\>"
			limit 'celf-font-evar-face))

; next two are now in celf.el
;(define-key celf-mode-map "\C-c\C-l" 'celf-font-fontify-decl)
;(define-key celf-mode-map "\C-cl" 'celf-font-fontify-buffer)
