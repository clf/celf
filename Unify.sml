...
fun occurObj ob =
    case SOME (lowerObj (NfObj.prj ob)) handle Subst.ExnUndef => NONE of
      NONE => NS.empty
...
and occurSub ctx s =
    let
      val () = if List.all (isSome o #3) $ ctx2list ctx then () else
    ...
    in
      Subst.fold (NS.union o (map1 occurSubOb)) (fn m => NS.occurFromTo (m+1) (ctxL-subL+m)) s
    end
and occurSpine sp =
...
fun synthSpine ctx sp ty =
    case (NfSpine.prj sp, Util.nfTypePrjAbbrev ty) of
      (Nil, _) => (ctx, ty)
    | (LApp (M, S), TLPi (_, _, B)) =>
...
and checkExistObj rOccur ob =
    let
      ...
      and chMo false lvCtrlCtx x = Util.NfMonadObjRec.map (chOb false lvCtrlCtx) x
        | chMo true lvCtrlCtx x =
          Util.NfMonadObjRec.fold (catch (chOb true lvCtrlCtx))
...
val (objExists, typeExists) =
    let 
      ...
      and pruneObj em rOccur srig x =
          NfObjRec.unfold (pruneSpine em rOccur, pruneExp em rOccur)
                          ...
                          (srig, x)
      and pruneExp em rOccur (srig, x) =
          NfExpObjRec.unfold (pruneSpine em rOccur, pruneMonad' em rOccur)
               ...
               | (LogicVar (X1 as {X=r, ty=A, s, ctx=ref (SOME G), cnstr=cs, tag}), _) =>
                 ...
                 in
                   case patSub s of
                     SOME (p1, s1) =>
                     let
                       val s2 = Subst.comp (Subst.coerce2s s1, Subst.lcis2sub p1)
...
fun unifyType em (ty1, ty2) =
    case (Util.nfTypePrjAbbrev ty1, Util.nfTypePrjAbbrev ty2) of
...
and unifySyncType em (ty1, ty2) =
    case (NfSyncType.prj ty1, NfSyncType.prj ty2) of
...
and unifyTSpine em (sp1, sp2) =
    case (NfTypeSpine.prj sp1, NfTypeSpine.prj sp2) of
      (TNil, TNil) => ()
...
and unifyHead errmsg dryRun (hS1 as (h1, S1), hS2 as (h2, S2)) =
    case (h1, h2) of
      (Const c1, Const c2) =>
    ...
    | (LogicVar (X1 as {X=r1, ty=A1, s=s1, ctx=ref (SOME G1), cnstr=cs1, tag=tag1}),
      ...
      if eq (r1, r2)
      then case (patSub s1, patSub s2) of
             (NONE, NONE) => (* FIXME: code restructuring? *)
             let
               val dryRunIntersect = ref true
             ...
             in
               if Subst.isId (Subst.intersection conv' (s1, s2))
                  handle ExnUnifyMaybe => false
               then ()
...
and unifyLVar errmsg (X as {X=r, s, cnstr=cs, tag, ...}, ob, p) =
    ...
    in
      case objExists errmsg r (NfClos (ob, si)) of
...
and unifySpine em dryRun (sp1, sp2) =
...
and unifyLetMon errmsg dryRun ((pa, hS, E), M) =
    case lowerAtomic hS of
...
and unifyLetLet errmsg dryRun ((p1, ob1, E1), (p2, ob2, E2)) =
    ...
    in
      case (ob1', NfExpObj.prj E1, ob2', NfExpObj.prj E2) of
        ...
        in case matchHeadInLet (ob1', E2t) of
           ...
           | INR m2 =>
             ...
             in case matchHeadInLet (ob2', E1t) of
                ...
                | INR m1 => if isSome dryRun
                            then (valOf dryRun) := false
                            else
                              let
                                fun notLvar (LogicVar _, _) = false
                                  | notLvar _ = true
                                fun lvarFreeN Et (_, 0) = true
                                  | lvarFreeN Et (E, n) =
                                    case lowerExp $ NfExpObj.prj E of
                              ...
                              in
                                if isSome m2 andalso lvarFreeN E1t (E2t, valOf m2) andalso notLvar ob1'
                                then unifyExp errmsg dryRun (E1, matchHeadInLetFixedPos errmsg (p1, ob1', E2t, valOf m2))
                                else if isSome m1 andalso lvarFreeN E2t (E1t, valOf m1) andalso notLvar ob2'
                                then  unifyExp errmsg dryRun (matchHeadInLetFixedPos errmsg (p2, ob2', E1t, valOf m1), E2)
...
and unifyLVarLetPrefix em (p1, X1 as {X, ty, s, ctx=ref G, ...}, p, E2) =
    let fun buildY ctx qn ex =
        ...
        fun splitLet ctx si ex qn =
            case NfExpObj.prj ex of
              NfMon _ => buildY ctx qn ex
            | NfLet (_, (LogicVar _, _), _) => buildY ctx qn ex
            | NfLet (q, hs, E) =>
              let
                fun prunehs' rX = (case objExists em rX (NfClos (NfAtomic' hs, si)) of
...
and matchHeadInLet' (hS, E) =
    let (* None n : No match found, looking at let number n
      ...
      fun foundOne (None l) = One l
        | foundOne (One _) = More
        | foundOne More = More
      fun matchHead (hS, e, nbe, E, EsX, nMaybe) =
          case (NfExpObj.prj E, NfExpObj.prj EsX) of
            ...
            let
              ...
              val e' = fn s => fn E =>
                                  let
                                    val s' = Subst.dotn nbe s
                                  in
                                    e s (NfLet' (p, AtClos (N, s'), E))
                                  end
              fun lVarSub (p, s) =
                  case Pattern.prj p of
              ...
              in
                ...
                then if isLVar N
                     then
                       ...
                       in
                         e (Subst.shift qn) (NfEClos (E'', Subst.switchSub (qn, nbe)))
                       end
            ...
            case lowerExp $ NfExpObj.prj E of
              ...
              let
                ...
                val e' = fn s => fn E =>
                                    let val s' = Subst.dotn nbe s
                                    in e s (NfLet' (p, AtClos (N, s'), E)) end
                fun assertMon () = case NfExpObj.prj E' of
                                     NfMon _ => ()
                                   | _ =>
                      ...
                      then ()
                        ...
                        BackTrack.backtrack (fn () =>
                                                let
                                                  ...
                                                  val (p', s') = valOf' $ patSub s
                                                in
                                                  case SOME (unifyLVarLetPrefix em (p, X with's s', p',
                                                  ...
                                                  | SOME (Y, qn, _) =>
                                                    let
                                                      val E'' = NfLet' (p, invAtomicP Y,
                                                                        NfEClos (E', Subst.dotn nbp (Subst.shift qn)))
                                                    in
                                                      sc (e (Subst.shift qn) (NfEClos (E'', Subst.switchSub (qn, nbe))))
                                                    end
                                                end))
                else
                  ( BackTrack.backtrack (fn () =>
...
fun unifyBranch errmsg (ob1, ob2, sc) =
    case (NfObj.prj ob1, NfObj.prj ob2) of
      (NfMonad E1, NfMonad E2) =>
...
fun unifyOpt (ty1, ty2, errmsg) =
    let
      val ty1 = Util.forceNormalizeType ty1
...
fun solveConstrBacktrack sc =
    let fun sc' () =
            if isSome $ solveAwakened ()
            then ()
            else BackTrack.backtrack (fn () => case !!monConstrs of
                                                 [] => sc ()
