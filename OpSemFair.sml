...
fun trace n s = if !traceSolve >= n then print s else ()

fun traceLeftFocus (h, ty) =
	trace 2 ("Trying " ^ PrettyPrint.printPreObj (Atomic' (h, Nil')) ^
...
fun solve (ctx, ty, sc) =
	( trace 3 ("Right Invert (" ^ PrettyPrint.printType ty ^ ")\n")
	; solve' (ctx, ty, sc) )
...
and matchAtom (ctx, P, sc) =
	(trace 2 ("Subgoal: MatchAtom (" ^ PrettyPrint.printType (AsyncType.inj P) ^ ")\n")
	; matchAtom' (ctx, P, sc) )
and matchAtom' (ctx, P, sc) =
	let val aP = (case P of TAtomic (a, _) => a
		...
		fun lFocus (ctx', lr, A, h) = fn () =>
					( traceLeftFocus (h, A)
					; leftFocus (lr, ctx', P', A, fn (S, ctxo) => (trace 2 "Succeeded\n";
										sc (Atomic' (h, S), ctxo)))
					; trace 2 "Failed\n" )
		fun matchSig (c, lr, A) = BackTrack.backtrack (lFocus (ctx, lr, A, Const c))

		val G = ctx2list $ #2 ctx
		fun matchCtx nil = ()
		  | matchCtx (k :: index) = 
		      (case List.nth (G, (k-1))  (* k = 1.. n, List.nth expects 0..n-1 *)
		       of (_, _, NONE) => matchCtx index
		        | (x, (A, hds), SOME modality) =>
		  	  let 
			    val ctx' = if modality=INT then ctx else removeHyp (ctx, k)
		...
		fun mkindex (nil, _) = nil
		  | mkindex (_ :: tl, n) = n :: mkindex (tl, n+1)  

	in
	  matchCtx (mkindex (G, 1))
...
and forwardChain (fcLim, ctx, S, sc) =
	(trace 2 ("ForwardChain ("^PrettyPrint.printType (TMonad' S)^")\n")
...
and rightFocus (ctx, m, sty, sc) =
	( trace 3 ("RightFocus (" ^ PrettyPrint.printType (TMonad' sty) ^ ")\n")
...
and leftFocus (lr, ctx, P, ty, sc) =
	( trace 3 ("LeftFocus (" ^ PrettyPrint.printType ty ^ ")\n")
...
and monLeftFocus (lr, ctx, ty, sc) =
	( trace 3 ("monLeftFocus (" ^ PrettyPrint.printType ty ^ ")\n")
