...
and pType ctx pa ty =
    if isUnknown ty
    then ["???"]
    else case AsyncType.prj ty of
	   TLPi (p, S, B) =>
           paren pa (case (Pattern.prj p, SyncType.prj S) of
		       (PBang NONE, TBang A) =>
...
and pSpineSkip ctx sp n =
    if n=0
    then pSpine ctx sp
    else case Spine.prj sp of
	   LApp (M, S) =>
